...
int main(int argc, char**argv){
        ...
        cout <<"path before command: mkdir -p "+path<<endl; 
        system("mkdir -p "+path);
        ...
        int deplow = 0;
                ...
                cout <<"ieta="<<ieta<<endl;
                        ...
                        if(abs(ieta)==16) {deplow=4;dephigh=4;}
                        if(abs(ieta)==17) {deplow=2;dephigh=3;}
                        if(abs(ieta)==18) {deplow=1;dephigh=5;}
                        if(abs(ieta)>=19 && abs(ieta)<=25 ) {deplow=1;dephigh=6;}
                        if(abs(ieta)==26) {deplow=1;dephigh=7;}
                        ...
                        for(int d=deplow;d<=dephigh;d++){
                                ...
                                TString cuttree("e"+Nidep+" < "+StrEmax+" && e"+Nidep+" > 0.0 && pt> " + ptcut + " && Trkmatch && HCal_ieta=="+Nieta+nvtxcut + " && HCal_iphi=="+Niphi);
                                if(Niphi.Contains("all")) cuttree = "e"+Nidep+" < "+ StrEmax +" && e"+Nidep+" > 0.0 && pt> " + ptcut + " && Trkmatch && HCal_ieta=="+Nieta+nvtxcut;
                                ...
                                RooRealVar* event = new RooRealVar("event", "event",10, 0, 1000000000000);
                                ...
                                RooRealVar* HCal_ieta = new RooRealVar("HCal_ieta","HCal_ieta",10,-40,40) ;
                                ...
                                RooRealVar* e4 = new RooRealVar("e"+Nidep,"e"+Nidep,1,0,Emax) ;
                                ...
                                RooAbsPdf* model = new RooAddPdf("model","signal + bkg",RooArgList(*bkg,*sig),*bkgfrac) ;
